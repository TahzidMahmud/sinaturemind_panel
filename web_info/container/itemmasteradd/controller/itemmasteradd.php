<?php
class Itemmasteradd extends Controller{
    ...
    function init(){
        ...
        $this->view->render("menutemplate","appsettings/itemmasteradd_view",false);
    ...
    function script(){
                 ...
                 success : function(result) {
                        ...
                        result.forEach(function(resp){
                            ...
                            <tr>                                                       
                                ...
                                <a href=\"javascript:void(0)\" id=\"`+resp.itemid+`act\" onclick=\"makeinactive(`+resp.itemid+`)\" class=\"mr-2\">`+(resp.active==1?'Make Inactive': 'Make Active')+`</a><br>
                                <a href=\"javascript:void(0)\" class=\"mr-2 text-primary\" onclick=\"openmodal2('`+resp.itemid+`','`+resp.mrp+`','`+resp.salesprice+`','`+resp.ablpercent+`','`+resp.longdesc+`')\" >Edit Item</a>
        ...
        $('#srcfbtn').on('click', function(e){
            ...
            $.ajax({
                ...
                url:\"".URL."?page=additems&action=searchitems\", 
                type : 'POST',  
                dataType:'json',                     				
                data: {
                    cat : cat,
                ...
                success : function(result) {
                   ...
                   if(result['message']=='success'){
                        Toast.fire({
                            icon: 'success',
                            title: 'search was successfull!'
                        })
                      if(resultobj.length < 1){
                    $('#fsearchresult').html(\"\");
                    $('#fsearchresult').append(`  NO Items Found To Add..!! `);
                  }else{
                   table.clear().draw();
                    drawtable()
                  }
